...
include 'base.php';
 ...
 $string = curl_get_contents("http://mp3quran.net/api/_arabic.php");
...
for($i = 0; $i < $len; $i++) {
    ...
    $temp = array();
     ...
     $temp[description] =   $count .' سورة رواية  ( ' . $rewaya .' )'  ;
      // $temp[html] = $row["html"];
       $temp[obj_url] =  $baseUrl . "/mp3Quran_ver2.php?id=" . $id;
       $temp[obj_free] = "yes" ;
       $temp[type]= type_open ;
...
{
      $string = curl_get_contents(  "http://api.4topapps.com/APPS/tsbeh/v3/suranames.json");
