...
function GetListByPage($Page , $categid,$baseUrl){
      $Page = $Page *10;
      $sql = "select DISTINCT categid, categ from firstinislam    ";
       
       if ($categid <> "") {
               $sql = "select * from firstinislam where categid=$categid  ORDER BY id ASC  limit $Page,10";
       }
       else
       {
     ...
     //  $temp[obj_photo] = $row['photo'];
      ...
      // $temp[html] = $row["html"];
        if ($categid <> "") {
                 ...
                 $temp[obj_url] =  $baseUrl . "/firstinislam.php?id=" . $row['id'];
        }
        else
        {
                 $temp[subtype]= subtype_list ;
                 $temp[obj_title] =  $row['categ'];
              $temp[obj_url] =  $baseUrl . "/firstinislam.php?categid=" . $row['categid'];   
        }
...
function GetRowByid($qid,$baseUrl){
             ...
             $sql = "select * from firstinislam where id=$qid   ";
